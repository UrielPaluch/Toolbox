...
from toolbox import calculo_plazo_liquidacion_24hs
...
def test_calculo_plazo_liquidacion_48hs():
    lunes = '2023-03-20'
    lunes_expected_return = 2
    lunes_actual_return = calculo_plazo_liquidacion_48hs(datetime.datetime.strptime(lunes,
                                                                               "%Y-%m-%d").date())
    ...
    jueves_actual_return = calculo_plazo_liquidacion_48hs(datetime.datetime.strptime(jueves,
                                                                                "%Y-%m-%d").date())
    ...
    viernes_actual_return = calculo_plazo_liquidacion_48hs(datetime.datetime.strptime(viernes,
                                                                                 "%Y-%m-%d").date())
    ...
    feriado_actual_return = calculo_plazo_liquidacion_48hs(datetime.datetime.strptime(feriado,
                                                                                 "%Y-%m-%d").date())

    assert feriado_expected_return == feriado_actual_return, "Fallo el feriado"

test_calculo_plazo_liquidacion_48hs()

def test_calculo_plazo_liquidacion_24hs():
    lunes = '2023-03-20'
    lunes_expected_return = 1
    lunes_actual_return = calculo_plazo_liquidacion_24hs(datetime.datetime.strptime(lunes,
                                                                               "%Y-%m-%d").date())

    assert lunes_expected_return == lunes_actual_return, "Fallo el lunes"

    jueves = '2023-03-30'
    jueves_expected_return = 1
    jueves_actual_return = calculo_plazo_liquidacion_24hs(datetime.datetime.strptime(jueves,
                                                                                "%Y-%m-%d").date())

    assert jueves_expected_return == jueves_actual_return, "Fallo el jueves"

    viernes = '2023-03-31'
    viernes_expected_return = 3
    viernes_actual_return = calculo_plazo_liquidacion_24hs(datetime.datetime.strptime(viernes,
                                                                                 "%Y-%m-%d").date())

    assert viernes_expected_return == viernes_actual_return, "Fallo el viernes"

    feriado = '2023-03-23'
    feriado_expected_return = 4
    feriado_actual_return = calculo_plazo_liquidacion_24hs(datetime.datetime.strptime(feriado,
                                                                                 "%Y-%m-%d").date())

    assert feriado_expected_return == feriado_actual_return, "Fallo el feriado"

test_calculo_plazo_liquidacion_24hs()
